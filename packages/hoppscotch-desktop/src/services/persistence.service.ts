...
export class DesktopPersistenceService {
  ...
  async setRecentInstances(instances: RecentInstance[]): Promise<void> {
    const result = await Store.set(
      STORE_NAMESPACE,
  ...
  async getRecentInstances(): Promise<RecentInstance[]> {
    const result = await Store.get<RecentInstance[]>(
      STORE_NAMESPACE,
      STORE_KEYS.RECENT_INSTANCES
    )
    if (E.isRight(result) && result.right) {
  ...
  async addRecentInstance(instance: RecentInstance): Promise<void> {
    const instances = await this.getRecentInstances()
    const existingIndex = instances.findIndex(
      (i) => i.serverUrl === instance.serverUrl
    )
