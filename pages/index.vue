  ...
  <div class="page">
      ...
      <ul>
        <li>
          ...
          <div v-if="response.body && responseType === 'text/html'" class="align-right">
            ...
            >{{ previewEnabled ? 'Hide Preview' : 'Preview HTML' }}</button>
          </div>
        </li>
      </ul>
    </pw-section>
    <pw-section class="gray" label="History">
      <ul>
        <li>
          <button
            v-bind:class="{ disabled: noHistoryToClear }"
...
const findStatusGroup = responseStatus =>
...
export default {
  ...
  data() {
    return {
      ...
      editor: new Editor({
        editable: false,
        content: `(waiting to send request)`,
        extensions: [
  ...
  watch: {
    "response.body"(content) {
      this.editor.setContent(`<pre><code>${content}</code></pre>`);
    }
  },
  computed: {
    statusCategory() {
