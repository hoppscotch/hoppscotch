 ...
 *   - pm.response.to.be.info()        → passes for 1xx status codes
 *   - pm.response.to.be.redirection() → passes for 3xx status codes
 *   - pm.response.to.be.error()       → passes for 4xx and 5xx status codes
 *   - pm.response.to.be.withBody()    → passes when response body is non-empty
 */
