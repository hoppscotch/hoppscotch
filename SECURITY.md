...
- **Self-hosted web client**: the web frontend served by a self-hosted instance.
...
**Bundle signature verification.** Remote bundles from self-hosted instances are verified with Ed25519 signatures and per-file BLAKE3 hashes. A bundle with an invalid signature or hash mismatch is rejected and will not load. The signing key is provided by the serving instance over TLS, so signature verification protects against bundle tampering in transit and corruption in the local cache. It does not protect against a compromised instance, since the instance provides both the key and the bundle. The trust boundary is the TLS connection to the instance and the user's decision to add it.

**Script sandboxing.** Pre-request and post-request scripts are isolated from the host environment. On the Desktop app, scripts run in a QuickJS WebAssembly sandbox, isolated from the browser context, the Tauri IPC layer, and the host OS. On the web client, scripts execute in dedicated Web Workers using the browser's own process isolation. In both cases, scripts receive read-only access to request properties via controlled API namespaces (`pw`, `hopp`, `pm`). Network access is mediated through a controlled fetch hook; scripts cannot make arbitrary system calls or access the filesystem. This sandbox applies equally to scripts imported from external collection files.
...
- WebSocket, SSE, Socket.IO, or MQTT connections reaching user-specified endpoints, including internal addresses. Same trust model as HTTP relay requests.
