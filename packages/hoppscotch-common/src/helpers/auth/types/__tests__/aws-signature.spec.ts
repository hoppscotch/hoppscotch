...
vi.mock("~/platform/std/kernel-interceptors/agent/store", () => ({
  KernelInterceptorAgentStore: "KernelInterceptorAgentStore",
}))
...
describe("AWS Signature Auth", () => {
  ...
  describe("profile credential mode", () => {
    ...
    test("should throw when profileName is empty in profile mode", async () => {
      const { AwsV4Signer } = await import("aws4fetch")
      ...
      ).rejects.toThrow("No AWS profile selected")

      // It must not silently fall back to (possibly empty/stale) manual keys.
      expect(mockResolveAwsCredentials).not.toHaveBeenCalled()
      expect(vi.mocked(AwsV4Signer)).not.toHaveBeenCalled()
    })

    test("should sign with the profile's resolved region when region is empty", async () => {
      const { AwsV4Signer } = await import("aws4fetch")
      ...
      // region field left blank -> agent called without a region override...
      expect(mockResolveAwsCredentials).toHaveBeenCalledWith(
        "my-profile",
        undefined
      )
      // ...and signing uses the profile's own region, not us-east-1.
      expect(vi.mocked(AwsV4Signer)).toHaveBeenCalledWith(
        expect.objectContaining({ region: "eu-west-1" })
      )
    })

    test("should prefer an explicit region over the profile's resolved region", async () => {
      const { AwsV4Signer } = await import("aws4fetch")
